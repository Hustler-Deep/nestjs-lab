...
import { Messages, messages, StandardSuccessResponse } from '@nestjs/shared-lib';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';

type ResponseShape<T> = { message?: Messages; data: T } | { message?: Messages } | T;
  ...
  intercept(
    ...
    return next.handle().pipe(
      map((originalData: ResponseShape<T>): StandardSuccessResponse<T> => {
        ...
        if (typeof originalData === 'object' && originalData !== null && 'data' in originalData) {
